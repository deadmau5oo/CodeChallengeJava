...
public class Challenge9 {
    ...
    public static int maxProfit(int[] prices) {
        ...
        int counter = 0;
        boolean action = true;
        ans = maxProfit(prices, ans, counter, action);
        return ans;
    }
    public static int maxProfit(int[] prices, int ans, int counter, boolean action) {
        if(counter==prices.length-1){
            return ans = action ? ans: ans+prices[counter];
        }
        else{
            if(counter<prices.length-1){
                if (action) {
        ...
        return maxProfit(prices, ans, counter+1, action);
    }
