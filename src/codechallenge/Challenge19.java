...
next element in S should be A[A[i]], and then A[A[A[i]]]… By that analogy, we stop adding right before 
...
public class Challenge19 {
    public static int arrayNesting(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            max = Math.max(max, maxLengthPosition(nums, i));
        }
        return max;
    }
    static int maxLengthPosition(int[] nums, int startIn){
        int temp = 0, index = startIn, counter = 0;
        while (nums[index] != -99) {            
            temp = nums[index];
            nums[index] = -99;
            index = temp;
    ...
    static void run(){
        int[] ArrayNums = {0,2,1};
        System.out.println(arrayNesting(ArrayNums));
    }
}
